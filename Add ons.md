...
A meeting scheduling system built directly into the HRMS that uses the Google Calendar API to create events and auto-generate Google Meet video call links. When an admin or HR officer schedules a meeting from within VibeHR, the system creates a Google Calendar event behind the scenes, attaches a Meet link, saves the meeting record in our database, and sends email invites to all selected attendees with the join link. No one needs to leave the app.
