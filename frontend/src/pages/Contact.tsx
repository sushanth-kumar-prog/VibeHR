            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
              {!sent ? (
                <form onSubmit={e=>{e.preventDefault(); setSent(true)}} className="space-y-4">
                  ...
                  <button className="w-full inline-flex items-center justify-center h-10 rounded-full bg-[#714B67] text-white text-sm font-medium"><Send className="mr-2 h-4 w-4" /> Send message</button>
                </form>
              ) : (
                <div className="text-center py-8">
                  ...
                  <Link to="/" className="mt-4 inline-flex text-sm text-[#714B67] font-medium">Back to home →</Link>
                </div>
              )}
            </div>
