...
export default function Verify() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#3d1f36] via-[#5a2e4d] to-[#714B67] p-4">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl ring-1 ring-black/5 overflow-hidden">
        <div className="h-1 bg-gradient-to-r from-[#714B67] via-[#b58aa6] to-[#e0b64c]" />
        <div className="p-8 text-center">
          <Link to="/" className="inline-flex items-center gap-2 mb-6">
            <img src="/logo.svg" alt="VibeHR logo" className="h-9 w-9 rounded-xl" />
            <span className="font-bold text-xl tracking-tight text-zinc-900 dark:text-white">VibeHR</span>
          </Link>

          {state === 'verifying' && (
            <>
              <Loader2 className="h-12 w-12 mx-auto animate-spin text-[#714B67]" />
          ...
          {state === 'success' && (
            <>
              ...
              <Link
                to="/login"
                className="mt-6 inline-flex items-center justify-center w-full h-11 rounded-xl bg-[#714B67] hover:bg-[#5d3d55] text-white text-sm font-semibold transition-colors"
              >
                Continue to Sign In
              </Link>
            </>
              ...
              {resent ? (
                ...
                <div className="mt-6 space-y-3 text-left">
                  ...
                  <input
                    ...
                    className="flex h-11 w-full rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900/50 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#714B67]/40"
                  />
                  <button
                    onClick={resend}
                    disabled={!email.trim() || resending}
                    className="w-full h-10 rounded-xl bg-[#714B67] hover:bg-[#5d3d55] disabled:opacity-50 text-white text-sm font-medium transition-colors"
                  >
